...
class CAFOClassifier(pl.LightningModule):
    ...
    def compute_losses(self, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # --- Resize masks to match S
        M_U_F = union_mask(mask)
        
        mask = F.interpolate(mask, size=S.shape[-2:], mode='nearest')  # [B, 7, H', W']
        
        # --- Construct unified mask
        M_U = union_mask(mask)  # [B, 1, H', W']
