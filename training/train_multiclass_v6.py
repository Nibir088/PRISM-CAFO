...
from model.classifierModule_v4 import CAFOClassifier
...
def main(config_file):
    ...
    model = CAFOClassifier(
        ...
        use_evidence_map=cfg['use_evidence_map'],
        use_component_pool=cfg['use_component_pool'],
        use_conf_gap=cfg['use_conf_gap'],
        use_concepts=cfg['use_concepts'],
        use_rel_feats=cfg['use_rel_feats'],
        linear_head=cfg['linear_head'],
        deepsets_hidden=cfg['deepsets_hidden'],
        freeze_backbone=cfg['freeze_model']
    ...
    checkpoint_cb = ModelCheckpoint(monitor="val_f1", mode="max", save_top_k=1, save_last=True)
    earlystop_cb = EarlyStopping(monitor="val_f1", patience=cfg["patience"], mode="max")
    ...
    if getattr(trainer, "is_global_zero", True):
        ...
        path = f"MODEL_Results/{cfg['model_name']}/{cfg['data_type']}"
        os.makedirs(path, exist_ok=True)

        output_path = f"MODEL_Results/{cfg['model_name']}/{cfg['data_type']}/spatial_{cfg['use_spatial_attn']}_pool_{cfg['use_attn_pooling']}_evidence_{cfg['use_evidence_map']}_component_{cfg['use_component_pool']}_confgap_{cfg['use_conf_gap']}_relfeature_{cfg['use_rel_feats']}_freeze_{cfg['freeze_model']}.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_file', default="config/config_fold1_v6.yaml")
