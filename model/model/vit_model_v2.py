...
from model.model.components.pooling import ComponentPooling, gap_conf, GAPConf
...
class ViTBuilder:
    ...
    def _build_vit_b_16(
        ...
        *,
        ...
        # Visual dimension (embed width)
        d_visual = vit.heads[-1].in_features if isinstance(vit.heads, nn.Sequential) else vit.heads.in_features
        deepsets_out = d_visual

        # Remove classification head; we’ll pool tokens ourselves
        vit.heads = nn.Identity()

        # Optional blocks (parity with Swin/ResNet/RemoteCLIP)
        ...
        # Classifier input dims depending on pooling path
        d_feat = d_visual
...
class ViTWrapper(nn.Module):
    """
    Mirrors Swin/ResNet/RemoteCLIP wrappers.
    ...
    def _tokens(self, x: torch.Tensor) -> torch.Tensor:
        """Tokenize image using torchvision ViT encoder; returns [B, N+1, D] including CLS."""
        B = x.size(0)
        vit = self.vit
        x = vit._process_input(x)                 # [B, N, D]
        cls = vit.class_token.expand(B, -1, -1)   # [B,1,D]
        x = torch.cat((cls, x), dim=1)            # [B, N+1, D]
        x = x + vit.encoder.pos_embedding         # [B, N+1, D]
        x = vit.encoder.dropout(x)
        x = vit.encoder.layers(x)                 # [B, N+1, D]
        x = vit.encoder.ln(x)                     # [B, N+1, D]
        return x
