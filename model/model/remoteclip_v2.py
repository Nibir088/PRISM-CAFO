    ...
    def _build_remoteclip_vit_b32(
        ...
        *,
        ...
        if not ckpt_path or not os.path.exists(ckpt_path):
            raise FileNotFoundError("RemoteCLIP checkpoint not found. Provide a valid `ckpt_path`.")

        # Create OpenCLIP ViT-B/32 and load RemoteCLIP weights
        model, _, _ = open_clip.create_model_and_transforms("ViT-B-32")
        sd = torch.load(ckpt_path, map_location="cpu")
        model.load_state_dict(sd, strict=True)

        visual = model.visual                       # OpenCLIP VisionTransformer
        d_visual = 768

        # Optional modules (mirrors Swin/ResNet builders)
        ...
        # Classifier input dims: deepsets_out only when using component pooling
        # (and not also using conf-GAP or mask-attn pooling). Otherwise d_visual.
        d_feat = d_visual
...
class RemoteCLIPWrapper(nn.Module):
    """
    Mirrors SwinWrapper/ResNetWrapper API.
    Expects batch:
      - 'rgb': [B,3,H,W]
      - optional 'masks': [B,K,H,W] (K=7)
      - optional 'mask_conf': [B,1,H,W]
    Returns self.last_out dict with:
    ...
    # ---- Patch-token feature map extraction (OpenCLIP ViT-B/32) ----
    def _extract_spatial_tokens(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x + v.positional_embedding.to(x.dtype)         # [B, N+1, C]
        x = v.ln_pre(x)                                    # [B, N+1, C]
        x = x.permute(1, 0, 2)                             # [N+1, B, C]
        x = v.transformer(x)                               # [N+1, B, C]
        x = x.permute(1, 0, 2)                             # [B, N+1, C]
        return x  # includes CLS token at index 0
    ...
    def forward(self, batch: Dict[str, Any], return_dict: bool = True):
        ...
        # Resize masks to token grid
        M7p = None
        if M7 is not None:
            M7p = F.interpolate(M7.float(), size=(Ht, Wt), mode="nearest")

        # Cross-attn + spatial gating
        ...
        # Optional simple relation priors (barn vs pond)
        R = None
